...
function Modal({ onClose, children, ...otherProps }) {
  const tabletSize = getComputedStyle(document.documentElement).getPropertyValue("--breakpoint-tablet").trim();
  ...
  if (!modalRoot) {
    console.error('#modal-root!');
    return null;
  }

  return createPortal(
    <div className={styles.backdrop} onClick={closeOnBackdrop}>
      <div className={styles.modal} {...otherProps}>
        {isMobile && <Header />}
          ...
          <button type="button" className={styles["btn-close"]} onClick={onClose}>
            {isMobile ?
              <svg alt="Close icon" width="12" className={styles["btn-close-icon"]}>
                <use href={`${btnClose}#btnCloseMob`} />
              </svg> :
              <svg alt="Close icon" width="12" className={styles["btn-close-icon"]}>
                <use href={`${btnClose}#btnClose`} />
              </svg>
            }  
          </button>
        </div>
